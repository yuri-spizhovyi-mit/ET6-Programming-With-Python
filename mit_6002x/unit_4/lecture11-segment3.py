...
def fitData(fileName):
    ...
    pylab.plot(
        ...
        estYVals,
        "r",
        label="Linear fit, r**2 = " + str(round(rSquared(yVals, estYVals), 5)),
    )
    ...
    pylab.plot(
        ...
        estYVals,
        "g--",
        label="Quadratic fit, r**2 = " + str(round(rSquared(yVals, estYVals), 5)),
    )
...
def getTempData():
    inFile = open("mit_6002x/unit_4/temperatures.csv")
    data = []
    inFile.readline()  # skip the header line: CITY,TEMP,DATE
    for l in inFile:
