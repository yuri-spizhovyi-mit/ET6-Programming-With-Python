...
SEARCH_URL = "https://www.linkedin.com/search/results/people/?geoUrn=%5B%22103644278%22%5D&keywords=data%20engineer&origin=GLOBAL_SEARCH_HEADER&sid=RE4"  # data engineer
NUM_PAGES = 8
...
async def extract_profiles_from_page(
    ...
    for url in profile_urls:
        ...
        try:
            ...
            # 🧠 Random view duration before interacting
            ...
            # 👇 Scroll down and up like a human with varying speed
            scroll_down_time = random.uniform(3, 7)
            down_start = datetime.now().timestamp()
            while datetime.now().timestamp() - down_start < scroll_down_time:
                await profile.mouse.wheel(0, random.randint(500, 1500))
                await asyncio.sleep(random.uniform(0.3, 0.7))

            await asyncio.sleep(random.uniform(0.5, 1.5))

            scroll_up_time = random.uniform(3, 7)
            up_start = datetime.now().timestamp()
            while datetime.now().timestamp() - up_start < scroll_up_time:
                await profile.mouse.wheel(0, random.randint(-1500, -500))
                await asyncio.sleep(random.uniform(0.3, 0.7))
