...
output_csv = "C:/Users/yspizhoviy/ET6-Programming-With-Python/linkedin/parsed_linkedin_output.csv"
...
def extract_education_details(matches):
    ...
    for text in matches:
        ...
        if any(
            k in lower
            for k in ["university", "college", "institute", "course", "courses"]
        ):
            school = text
        elif (
            "degree" in lower
            ...
            r"^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\s+\d{4}\s*-\s*(Present|\w+\s+\d{4})$",
            text,
        ):
            years = re.findall(r"\d{4}", text)
            if len(years) == 2:
